...
delta_t = (
    (load_df.iloc[-1]["Datetime"] - load_df.iloc[0]["Datetime"]) / T
...
for t in range(T):
    constraints += [
        battery_soc[t + 1]
        == battery_soc[t] + (battery_output_kW[t] * delta_t) / total_capacity,
...
fig.tight_layout()
plt.legend()
plt.savefig("cvx-cost-model-out.png", bbox_inches="tight")

# plot the battery charge
fig, ax = plt.subplots()
ax.step(
    charge_df["DateTime"], battery_soc.value[1:], color="C1", lw=2, label="Battery SOC"
)
ax.set(
    xlabel="Time",
    ylabel="Battery SOC",
...
fig.tight_layout()
plt.legend()
plt.savefig("cvx-emit-model-out.png", bbox_inches="tight")
